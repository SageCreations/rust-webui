...
use crate::bindings::{self, webui_event_t};
...
impl Event {
    /// Wrap a raw C event pointer. Called exclusively from the trampoline in
    /// callbacks.rs.
    ///
    /// # Safety
    /// `raw` must be a valid, non-null pointer to a `webui_event_t` that lives
    /// for the entire duration this `Event` object exists.
    pub(crate) unsafe fn from_raw(raw: *mut webui_event_t) -> Self {
        Event { raw }
    }
    ...
    pub fn get_string(&self) -> String {
        unsafe {
            let ptr = bindings::webui_get_string(self.raw);
            if ptr.is_null() {
                String::new()
            } else {
                CStr::from_ptr(ptr).to_string_lossy().into_owned()
    ...
    pub fn get_int_at(&self, index: usize) -> i64 {
        // bindgen maps `long long` to `c_longlong`; cast to the stable `i64`.
        unsafe { bindings::webui_get_int_at(self.raw, index) as i64 }
    }

    /// Get the first argument as an `i64`.
    pub fn get_int(&self) -> i64 {
        unsafe { bindings::webui_get_int(self.raw) as i64 }
    }

    /// Get the argument at `index` as an `f64`.
    pub fn get_float_at(&self, index: usize) -> f64 {
        unsafe { bindings::webui_get_float_at(self.raw, index) }
    }

    /// Get the first argument as an `f64`.
    pub fn get_float(&self) -> f64 {
        unsafe { bindings::webui_get_float(self.raw) }
    }

    /// Get the argument at `index` as a `bool`.
    pub fn get_bool_at(&self, index: usize) -> bool {
        unsafe { bindings::webui_get_bool_at(self.raw, index) }
    }

    /// Get the first argument as a `bool`.
    pub fn get_bool(&self) -> bool {
        unsafe { bindings::webui_get_bool(self.raw) }
    }

    /// Get the byte-size of the argument at `index`.
    pub fn get_size_at(&self, index: usize) -> usize {
        unsafe { bindings::webui_get_size_at(self.raw, index) }
    }

    /// Get the byte-size of the first argument.
    pub fn get_size(&self) -> usize {
        unsafe { bindings::webui_get_size(self.raw) }
    }
    ...
    pub fn return_int(&self, n: i64) {
        // bindgen expects `c_longlong`; on all supported platforms this is i64.
        unsafe { bindings::webui_return_int(self.raw, n as _) }
    }

    /// Send a float back to the JavaScript caller.
    pub fn return_float(&self, f: f64) {
        unsafe { bindings::webui_return_float(self.raw, f) }
    }

    /// Send a string back to the JavaScript caller.
    pub fn return_string(&self, s: &str) {
        let cstr = CString::new(s).unwrap_or_default();
        unsafe { bindings::webui_return_string(self.raw, cstr.as_ptr()) }
    }

    /// Send a boolean back to the JavaScript caller.
    pub fn return_bool(&self, b: bool) {
        unsafe { bindings::webui_return_bool(self.raw, b) }
    }
    ...
    pub fn run_client(&self, script: &str) {
        ...
        unsafe { bindings::webui_run_client(self.raw, cstr.as_ptr()) }
    }

    /// Run JavaScript on this specific client and return the result.
    ///
    /// Returns `Ok(response)` on success, `Err(ScriptError)` on execution error.
    pub fn script_client(&self, script: &str, timeout: Duration) -> Result<String, ScriptError> {
        let cstr = CString::new(script).unwrap_or_default();
        let buf_size = 4096;
        let mut buf: Vec<u8> = vec![0u8; buf_size];
        let ok = unsafe {
            bindings::webui_script_client(
                self.raw,
                cstr.as_ptr(),
    ...
    pub fn send_raw_client(&self, function: &str, data: &[u8]) {
        ...
        unsafe {
            bindings::webui_send_raw_client(
                self.raw,
                cstr.as_ptr(),
